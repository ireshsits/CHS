...
namespace App\Models\Repositories\Search;
...
class ManageSearchRepository {
    ...
    private function generateQuery($filters) {
        ...
        // ->join('complainants', 'complaints.complainant_id_fk', '=', 'complainants.complainant_id_pk');
...
//            $column = "";
//
//            $query->where('complainant', function($query) use ($column, $filters) {
//                    return $query->where('nic' . $column . '', 'LIKE', "%".$filters['search']."%");
//            });//->get();
//
//
//            $query->where('account_no' . $column . '', 'LIKE', "%".$filters['search']."%");
//
//                    // $column = "";
//                    // $query->whereHas('complainant', function ($query) use ($column, $filters) {
//                    // 	$query->where('complainants.nic' . $column . '', 'LIKE', "%".$filters['search']."%");///return $query;
//                    // })->orWhere(function ($query) use ($column, $filters) {
//                    // 	$query->where('complaints.account_no' . $column . '', 'LIKE', "%".$filters['search']."%");
//                    // });
//
//                    // $query->where('complainants.nic' . $column . '', 'LIKE', "%".$filters['search']."%");
//                    // $query->orWhere('complaints.account_no' . $column . '', 'LIKE', "%".$filters['search']."%");
//                    // $q->where('account_no', $filters['search']);
//                    // $q->orWhereHas('complainants.nic', function($qd) {
//                    // 	$qd->where('complainants', UserHelper::getLoggedUserId ());
//                    // });
//        }
        
//        $filters['ref_no'] || $filters['nic'] || $filters['account_no']

        // union
//        if(isset($filters['ref_no'])) {
//            $query->where('complaints.reference_number', 'like', '%'.$filters['ref_no'].'%');
//        }
//        
//        if(isset($filters['nic'])) {
//            $query->orWhereHas('complainant', function ($q) use ($filters){
//                $q->where('nic', 'LIKE', "%".$filters['nic']."%");
//            });
//        }
//        
//        if(isset($filters['account_no'])) {
//            $query->orWhere('complaints.account_no', 'like', '%'.$filters['account_no'].'%');
//        }
        // union ------------
        ...
        if(isset($filters['nic'])) {
            $query->WhereHas('complainant', function ($q) use ($filters){
                $q->where('nic', 'LIKE', "%".$filters['nic']."%");
            });
        }
        
        if(isset($filters['account_no'])) {
            $query->Where('complaints.account_no', 'like', '%'.$filters['account_no'].'%');
        }

        if(isset($filters['contact_no'])) {
            $query->WhereHas('complainant', function ($q) use ($filters){
                $q->where('contact_no', 'LIKE', "%".$filters['contact_no']."%");
            });
        }
...
//            $query->whereIn ('complaints.status', explode ( ',', $filters ['status'] ) );
    ...
    public function getComplaints($filters) {
        ...
        if ($filters['ref_no'] || $filters['nic'] || $filters['account_no'] || $filters['contact_no']) {
            $queryString = $this->generateQuery($filters);
        } else {
            $queryString = collect([]);
        }

        $datatable = DataTables::of($queryString)

        // ->filter(function ($instance) use ($filters) {
        // 	if (isset($filters['search'])) {
        // 		$instance->collection = $instance->collection->filter(function ($row) use ($filters) {
        // 			if ($row['account_no'] == $filters['search']) {
        // 				return true;
        // 			} else if ($row['nic'] == $filters['search']) {
        // 				return true;
        // 			}
        // 			return false;
        // 		});
        ...
        // 	$query->whereRaw('account_no', ["%{".$filters['search']."}%"]);
        // })
...
//        });
    ...
    private function generateSearchQuery($data) {
        ...
        if ($data['type'] == 'REF') {
            $query = Complaint::select('reference_number');
            return $query;
        } elseif ($data['type'] == 'NIC') {
            $query = Complainant::select('nic');
            return $query;
        } elseif ($data['type'] == 'ACCNO') {
            $query = Complaint::select('account_no')->whereNotNull('account_no');
            return $query;
        } elseif ($data['type'] == 'CNTNO') {
            $query = Complainant::select('contact_no')->whereNotNull('contact_no');
            return $query;
        }
        
    }
    
    private function buildFilterItemSearchResponse($item, $type) {
        
        if ($type == 'REF') {
            $item->id = $item->reference_number;
            $item->text = $item->reference_number;
            return $item;
        } elseif ($type == 'NIC') {
            $item->id = $item->nic;
            $item->text = $item->nic;
            ...
            $item->id = $item->account_no;
            $item->text = 'ACC-'.$item->account_no;
            return $item;
        } elseif ($type == 'CNTNO') {
            $item->id = $item->contact_no;
            $item->text = $item->contact_no;
            return $item;
        }
        
    }
    
    public function getFilterForSearch($data) {
        
        $queryString = $this->generateSearchQuery($data);
        $response = $queryString->get();
        foreach ($response as $item) {
            $item = $this->buildFilterItemSearchResponse($item, $data['type']);
        }
        return $response;
