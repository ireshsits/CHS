...
/***/ "./public/dashboard/js/reports/manage.js":
/*!***********************************************!*\
  !*** ./public/dashboard/js/reports/manage.js ***!
  \***********************************************/
...
var selects = ['#branch_department_id_fk', '#complaint_mode_id_fk', //				'#category_id_fk', '#sub_category_id_fk',
...
function initCategoryFilters() {
  ...
  if (categoryLevelCount > 4) var sizeClass = 'col-md-2';else if (categoryLevelCount > 3) var sizeClass = 'col-md-3';

  for (var i = 1; i <= categoryLevelCount; i++) {
    row += '<div class="' + sizeClass + ' col-xs-12">' + '<div class="form-group">' + '<label for="category_level_' + i + '">Category level ' + i + ':</label>' + '<select data-placeholder="Select Category level ' + i + '" data-index=' + i + ' style="width: 100% !important;" class="form-control select-category-search-' + i + ' select2-close category_id_fk-error error-placement" onselect="onSelect(this)" id="category_id_fk_' + i + '">' + '</select>' + '</div>' + '</div>';
  }
  ...
  $.each($dates, function (index, element) {
    $(element).on('change', function () {
      if (element == '#report_date_from') {
        if ($(this).val() !== '') filters.date_from = $(this).val();else filters.date_from = '';
      }

      if (element == '#report_date_to') {
        if ($(this).val() !== '') filters.date_to = $(this).val();else filters.date_to = '';
      }
    });
  });
...
function format(d) {
  ...
  return '<div class="slider" name>' + '<table class="detail-table" cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;width:100%">' + '<tr>' + '<td width="30%">Account Number:</td>' + '<td>' + d.account_no + '</td>' + '</tr>' + '<tr>' + '<td width="30%">Resolved By:</td>' + '<td>' + d.resolved_by + '</td>' + '</tr>' + '<tr>' + '<td width="30%">Event Closed Date:</td>' + '<td>' + d.close_date + '</td>' + '</tr>' + '<tr>' + '<td width="30%">1st Reminder:</td>' + '<td>' + d.reminder_1 + '</td>' + '</tr>' + '<tr>' + '<td width="30%">2nd Reminder:</td>' + '<td>' + d.reminder_2 + '</td>' + '</tr>' + '<tr>' + '<td width="30%">Final Reminder:</td>' + '<td>' + d.reminder_3 + '</td>' + '</tr>' + '<tr>' + '<td width="30%">Complaint:</td>' + '<td>' + decodeToHtml(d.complaint) + '</td>' + '</tr>' + '</table>' + '</div>';
}
    ...
    table = $(".reports-table").DataTable({
      ...
      ajax: {
        ...
        data: function data(d) {
          d.branch_department_id_fk = filters.branch_department_id_fk, d.complaint_mode_id_fk = filters.complaint_mode_id_fk, d.category_id_fk = filters.category_id_fk, //				  d.sub_category_id_fk = filters.sub_category_id_fk,
          d.area_id_fk = filters.area_id_fk, d.status = filters.status, d.date_from = filters.date_from, d.date_to = filters.date_to, d.type = filters.type;
        }
      },
      columns: [{
...
/***/ 25:
/*!*****************************************************!*\
  !*** multi ./public/dashboard/js/reports/manage.js ***!
  \*****************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! C:\wamp64\www\sits\complaintms_NSC_V1\public\dashboard\js\reports\manage.js */"./public/dashboard/js/reports/manage.js");
