...
/***/ "./public/dashboard/js/analysis/manage.js":
/*!************************************************!*\
  !*** ./public/dashboard/js/analysis/manage.js ***!
  \************************************************/
...
function eventHandler() {
  $(".detail-table").parents('td').css("text-align", "inherit"); //	$('#analysis-form').on('submit', function(e){
  //		e.preventDefault();
  //		initAnalysis();
  //	});
...
function emptyChartView() {
  ...
  if (typeof window.myPie !== 'undefined') window.myPie.destroy();
}
...
function initHeaders() {
  $.ajax({
    url: custom_url + '/dashboard/analysis/table_headers/' + encodeURIComponent($.param({
      year: filters.year,
      code: filters.code
    })),
    type: 'GET',
    ...
    success: function success(data) {
      if (data.status == "success" && data.component) $('.displayTable').empty().html(data.component).hide();
    ...
    table = $(".analysis-table").DataTable({
      ...
      ajax: {
        ...
        data: function data(d) {
          d.code = filters.code, d.year = filters.year, d.status = filters.status, d.area_id_fk = filters.area_id_fk, d.category_id_fk = filters.category_id_fk, d.type = filters.type;
        }
      },
      columns: columns,
      rowGroup: {
        ...
        startRender: function startRender(rows, group) {
          if (filters.code == 'AWODA' || filters.code == 'BSLZR') return group + ' (' + rows.count() + ' rows)';
        }
      },
      ...
      createdRow: function createdRow(nRow, aData, iDataIndex) {
        if (filters.code == 'RA' && iDataIndex == 4) $(nRow).addClass('total');
      },
      drawCallback: function drawCallback(settings) {
        if ($.inArray(filters.code, ['RA', 'TFCR']) == -1) {} else {
          var api = this.api();
          var rows = api.rows({
            page: 'current'
          }).nodes();
          ...
          api.column(0, {
            ...
            if (filters.code == 'TFCR') {
              ...
              if (i == 5 || i == 6 || i == 7) {
                $(rows).eq(i).before('<tr class=""><td colspan="10">' + group + '</td><td>' + $(rows).eq(i).find("td:last").text() + '</td></tr>');
                $(rows).eq(i).remove();
              }
            }
          });
...
function generateRandomColors(length) {
  ...
  return colors; //	var colorsArr = ['#1ABC9C','#2ECC71','#3498DB','#9B59B6','#34495E','#F1C40F','#E67E22','#E74C3C','#6D6FA7','#95A5A6','#16A085','#27AE60','#2980B9','#8E44AD',
...
updateDataset = function updateDataset(e, datasetIndex) {
...
/***/ 7:
/*!******************************************************!*\
  !*** multi ./public/dashboard/js/analysis/manage.js ***!
  \******************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! C:\wamp64\www\sits\complaintms_NSC_V1\public\dashboard\js\analysis\manage.js */"./public/dashboard/js/analysis/manage.js");
